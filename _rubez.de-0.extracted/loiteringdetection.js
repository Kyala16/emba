...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'confidenceval', 'div_loiteringdetection_confidence_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'showid', 'label_loiteringdetection_showid_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'targetmask', 'label_loiteringdetection_tagmask_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'scenemode', 'div_loiteringdetection_scene_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'minstay', 'div_loiteringdetection_remain_name', ITEM_TYPE_TEXT);
  ...
  var strsecond = translate_page_item(TARGET_PAGE_COMMON, 'second', '', ITEM_TYPE_NONE);
  $('#select_loiteringdetection_remain_mode option').each(function (i, n) {
    if (i == 0) {
      $(n).text('5' + strsecond);
    } else if (i == 1) {
      $(n).text('10' + strsecond);
...
function fun_init_draw_panel() {
  $('#slider_loiteringdetection_confidence').slider({
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=15', function (result) {
    if (result != false) {
      $xml = $(result);
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=loitering', function (result) {
    if (result === false) {
      return;
    }
    $xml = $(result);
    var staytime = $xml.find('second').text();
    if (staytime === '5' || staytime === '10' || staytime === '15') {
      $('#select_loiteringdetection_remain_mode').val(staytime);
    }
    var ptindex = 0;
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=15', targetxml, function (result) {
...
function fun_on_base_save() {
  var points = null;
  var staytime = $('#select_loiteringdetection_remain_mode').val();
  if (CurBrowserIsIE()) {
    if (null !== g_activex_plugin) {
  ...
  sdk_setipcparam('/action/set?subject=loitering', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);
...
function fun_on_region_restore() {
  $('#select_loiteringdetection_remain_mode').val(5);
  if (CurBrowserIsIE()) {
    if (null !== g_activex_plugin) {
...
function fun_on_slider_change() {
  var value = $('#slider_loiteringdetection_confidence').slider('option', 'value');
  $('#div_loiteringdetection_confidence_value').text(value);
}

...
function fun_get_ability() {
  ...
  sdk_getipcparam('/action/get?subject=devability', function (result) {
    if (result != false) {
      ...
      //===================================Ai  START===================================
      const mdtype = parseInt($xml.find('mdtype').text());
      // eval(`$("#div_crossline_type,#div_crossline_double_line").${mdtype ? 'show' : 'hide'}()`)
      eval(`$("#div_crossline_type").${mdtype ? 'show' : 'hide'}()`)

      var crossline_line = $xml.find('crossline_line').text();
      $('#div_crossline_double_line').css('display',crossline_line=='2'?'block':'none');
...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'sensitivity', 'div_crossline_sensitivity_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'targetblink', 'label_crossline_trigger_blink_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'scenemode', 'div_crossline_scene_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'linedirection', 'div_crossline_derection_name', ITEM_TYPE_TEXT);
  ...
  // ====================================AI START===========================================================
  translate_page_item(TARGET_PAGE_COMMON, 'action', 'label_crossline_type_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_MOTION, 'smarthuman', 'label_crossline_type_human_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_MOTION, 'smartvehicle', 'label_crossline_type_vehicle_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'enabledoubleline', 'label_crossline_double_line_name', ITEM_TYPE_TEXT);
  // ====================================AI START===========================================================
  ...
  $('#select_crossline_scene_mode option').each(function (i, n) {
    if (i === 0) {
      $(n).text(indoor);
    } else if (i === 1) {
      $(n).text(outdoor);
...
function fun_register_all_events() {
  ...
  $('#check_crossline_double_line').change(function () {
    lineNum = $(this).is(':checked') ? 2 : 1;
    isLine2 = false;
    fun_on_base_clear();//clean 
  });
}
...
function fun_facedetect(person, enable, conf, size, start, end) {
  this.person = person;
  this.enable = enable;
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=10', function (result) {
    if (result != false) {
      $xml = $(result);
      ...
      $('#check_action_motionemulation').prop('checked',enumlation=='1'?true:false);
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=crossline', function (res) {
    if (res === false) return;
    $xml = $(res);
    var beginpt = $xml.find('begin').text();
    var endpt = $xml.find('end').text();
    var sensiti = $xml.find('sensitivity').text();
    var direction = $xml.find('direction').text();
    var scenemode = $xml.find('scene').text();
    var blink = $xml.find('blink').text();
    var pt1, pt2;
    ...
    // if (null !== video_overlay_draw) {
    //   video_overlay_draw.setvalue(pt1, pt2);
    // }
    sensiti = parseInt(sensiti);
    if (sensiti >= 1 && sensiti <= 4) {
      $('#select_crossline_sensitivity').val(sensiti);
    }
    direction = parseInt(direction);
    if (direction >= 0 && direction <= 2) {
      $('#select_crossline_derection').val(direction);
    }
    scenemode = parseInt(scenemode);
    if (scenemode >= 0 && scenemode <= 1) {
      $('#select_crossline_scene_mode').val(scenemode);
    }
    if (blink === '1') {
      $('#check_crossline_trigger_blink').prop('checked', true);
    } else {
      $('#check_crossline_trigger_blink').prop('checked', false);
    }

    //-----------------Ai  -------------------Ai
    const mdsmart = parseInt($xml.find('mdsmart').text());
    g_mdsmart = mdsmart;
    switch (mdsmart) {
      case 1:
        $('#check_crossline_type_human').attr('checked', 'true')
        break;
      case 2:
        $('#check_crossline_type_vehicle').attr('checked', 'true')
        break;
      case 3:
        $('#check_crossline_type_human,#check_crossline_type_vehicle').attr('checked', 'true')
        break;
    }

    const LineNum = parseInt($xml.find('num').text());
    // $('#check_crossline_double_line').val(LineNum == 2);
    $('#check_crossline_double_line').prop('checked', LineNum == 2);
    if (LineNum == 2) {
      lineNum = 2;
      isLine2 = false;
      fun_on_base_clear();//clean 
    ...
    // ????----------
    var beginstr2 = $xml.find('begin2').text().split(',');
    var endstr2 = $xml.find('end2').text().split(',');
    var startpoint2 = new fun_point(beginstr2[0], beginstr2[1]);
    var endpoint2 = new fun_point(endstr2[0], endstr2[1]);
    if (null !== video_overlay_draw) {
      if (lineNum == 2) {
        video_overlay_draw.setvalue(pt1, pt2, startpoint2, endpoint2);
      } else {
        video_overlay_draw.setvalue(pt1, pt2, null, null);
      }
    }
    //-----------------Ai  -------------------Ai
  });
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=10', targetxml, function (result) {
...
function fun_on_base_save() {
  var sensitivity, direction, scenemode, blink;
  // if (null !== video_overlay_draw) {
  //   var result = video_overlay_draw.getvalue();
  //   startpt = result.a;
  //   endpt = result.b;
  // }
  sensitivity = $('#select_crossline_sensitivity').val();
  direction = $('#select_crossline_derection').val();
  scenemode = $('#select_crossline_scene_mode').val();
  blink = $('#check_crossline_trigger_blink').prop('checked') ? 1 : 0;

  //-------------------ai----------------Ai
  //  const Limittype = $('#check_crossline_limit_type').prop('checked') ? 1 : 0;
  //  const type = $('#select_crossline_type').val();
  const num = $('#check_crossline_double_line').prop('checked') ? 2 : 1
  var gtips_input = translate_page_item(TARGET_PAGE_TIPSTEXT, "errset", "", ITEM_TYPE_NONE);
  let begin2 = '0,0'
  let end2 = '0,0';
  let beginpt, endpt
  const drawresult = video_overlay_draw.getvalue();
  ...
  if (num == '2') {//?????
    ...
    if (isLinesIntersect(a.x, a.y, b.x, b.y, c.x, c.y, d.x, d.y)) {//?��??????????????
      parent.fun_show_tips_dialog(gtips_input, 0);
      return
    }
  }
  var check_human = $('#check_crossline_type_human').is(':checked');
  var check_vehicle = $('#check_crossline_type_vehicle').is(':checked');
  var mdsmart = 1;
  if (check_human && check_vehicle) {//is check vehicle & human
    ...
    mdsmart = 1;
  } else if (!check_human && !check_vehicle&&$('#div_crossline_type').is(':visible')) {//no check vehicle & human
    parent.fun_show_tips_dialog(gtips_input, 0);
    return;
  }
  if ((g_mdsmart == 1) && ($("#check_crossline_type_human").is(":hidden")) && mdsmart != 3) {
    parent.fun_show_tips_dialog(gtips_input, 0);
  }
  if ((g_mdsmart == 2) && ($("#check_crossline_type_vehicle").is(":hidden")) && mdsmart != 3) {
    parent.fun_show_tips_dialog(gtips_input, 0);
  }
  //-------------------ai----------------Ai
  ...
  var targetxml =
    ...
    '<scene>' + scenemode + '</scene>\n' +
    '<blink>' + blink + '</blink>\n' +
    '</crossline>\n' +
    '</request>';

  sdk_setipcparam('/action/set?subject=crossline', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);
...
function fun_on_region_restore() {
  $('#check_crossline_trigger_blink').prop('checked', true);
  $('#select_crossline_sensitivity').val(3);
  $('#select_crossline_derection').val(0);
  $('#select_crossline_scene_mode').val(0);
  if (null !== video_overlay_draw) {

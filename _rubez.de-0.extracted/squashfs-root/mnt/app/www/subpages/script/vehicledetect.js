...
function fun_init_uicomponents() {
  sdk_getipcparam('/action/get?subject=devability', function (result) {
    ...
    $xml = $(result);
    var smartvaalg = parseInt($xml.find('smartva_alg').text());
    smartvaalg = 7
    if (smartvaalg === 7) {
      $('#div_humandct_scene').css('display', 'none');
      //$('#div_humandct_showid').css('display', 'block');
      //$('#button_region_clear').css('display', 'none');
    ...
    var aout = $xml.find('aout').text();
    if (ioout > 0) {
      $('#div_page_action_iooutput').css('display', 'block');
    } else {
      $('#div_page_action_iooutput').css('display', 'none');
    }
    if (aout > 0) {
      $('#div_page_action_audioout').css('display', 'block');
    } else {
      $('#div_page_action_audioout').css('display', 'none');
    }
    var disk = parseInt($xml.find('disk').text());
    if (disk > 0) {
      $('#div_page_action_record').css('display', 'block');
    } else {
      $('#div_page_action_record').css('display', 'none');
    }
  });
...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'confidenceval', 'div_humandct_confidence_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'showid', 'label_humandct_showid_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'targetmask', 'label_humandct_tagmask_name_vehicle', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'scenemode', 'div_humandct_scene_name', ITEM_TYPE_TEXT);
  ...
  fase_detect_note = translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'facenote', '', ITEM_TYPE_NONE);
  ...
  var indoorhuman = translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'indoorhuman', '', ITEM_TYPE_NONE);
  var outdoorhuman = translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'outdoorhuman', '', ITEM_TYPE_NONE);
  var outcar = translate_page_item(TARGET_PAGE_SUB_BONJOUR, 'humanandcar', '', ITEM_TYPE_NONE);
  $('#select_humandct_scene_mode option').each(function (i, n) {
    if (i === 0) {
      $(n).text(indoorhuman);
    } else if (i === 1) {
      $(n).text(outdoorhuman);
...
function fun_register_all_events() {
  ...
  $('#check_humandct_tagmask').change(function () {
    if (g_showmask != 1) {
      return;
    }
    var val = $('#check_humandct_tagmask').prop('checked');
    if (val == 0) {
      $('#check_humandct_showid').prop('disabled', true);
      $('#check_humandct_showid').prop('checked', false);
    } else {
      $('#check_humandct_showid').prop('disabled', false);
    }
  });
}
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=25', function (result) {
    if (result != false) {
      $xml = $(result);
...
var g_rowcount, g_colcount, g_showmask;

function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=vehicledetect', function (result) {
    if (result === false) {
      return;
    }
    $xml = $(result);
    ...
    var scene = 0,
      showid = 0;
    if (g_algorithm_type === 2) {
      scene = $xml.find('scene').text();
    } else if (g_algorithm_type === 7) {
      scene_ = $xml.find('scene').text();
      if (scene_ == -1) {
        scene = -1;
      }
    }
    showid = $xml.find('showid').text();
    var showrec = $xml.find('border').text();
    var marker = $xml.find('marker').text();
    ...
    g_showmask = marker;
    if (marker == 1) {
      $('#div_humandct_tagmask').css('display', 'block');
    } else {
      $('#div_humandct_tagmask').css('display', 'none');
    }
    if (marker == 1 && showrec < 1) {
      $('#check_humandct_showid').prop('disabled', true);
    } else {
      $('#check_humandct_showid').prop('disabled', false);
    }
    if (showid == 1) {
      $('#check_humandct_showid').prop('checked', true);
    } else {
      $('#check_humandct_showid').prop('checked', false);
    }
    if (parseInt(scene) >= 0 && parseInt(scene) <= 2) {
      $('#select_humandct_scene_mode').val(scene);
    }
    if (parseInt(scene) == -1) {
      $('#div_humandct_scene').css('display', 'none');
    }
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=25', targetxml, function (result) {
...
function fun_on_base_save() {
  var drawmask, showmask, confidence, showid, scenemode;
  var points = null;
  ...
  showmask = $('#check_humandct_tagmask').prop('checked') ? 1 : 0;
  showid = $('#check_humandct_showid').prop('checked') ? 1 : 0;
  confidence = $('#slider_humandct_confidence').slider('value');
  scenemode = $('#select_humandct_scene_mode').val();
  ...
  if (g_algorithm_type === 2) {
    targetxml =
      '<?xml version="1.0" encoding="utf-8"?>' +
      '<request>' +
      '<vehicledetect ver="2.0">' +
      '<enable>1</enable>' +
      '<confidence>' +confidence +'</confidence>' +
      '<scene>' +scenemode +'</scene>' +
      '<border>' +showmask +'</border>' +
      '<showid>' + showid +'</showid>' +
      '<polygon>\n' +
      '<point>' +points[0].x + ',' +points[0].y +'</point>\n' +
      '<point>' +points[1].x +',' +points[1].y +'</point>\n' +
      '<point>' + points[2].x +',' +points[2].y +'</point>\n' +
      '<point>' + points[3].x +',' +points[3].y +'</point>\n' +
      '</polygon>\n' +
      '</vehicledetect>' +
      '</request>';
  } else if (g_algorithm_type === 7) {
  ...
  sdk_setipcparam('/action/set?subject=vehicledetect', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);
...
function fun_on_region_restore() {
  // console.log(video_overlay_draw);
  $('#check_humandct_tagmask').prop('checked', false);
  if (g_showmask == 1) {
    $('#check_humandct_showid').prop('disabled', true);
  }
  $('#check_humandct_showid').prop('checked', false);
  $('#slider_humandct_confidence').slider('value', 50);//滑动�?
  $('#select_humandct_scene_mode').val(0);
  // if (CurBrowserIsIE()) {
  //   if (null !== g_activex_plugin) {
  //     g_activex_plugin.ClearDrawItems();
  //   }
...
function fun_on_slider_change() {
  ...
  $('#div_humandct_confidence_value').text(value);
}

function fun_on_button_fullscreen() {//全屏
  if (CurBrowserIsIE()) {
    // g_activex_plugin.SetPiecesMask('ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffff00');
    g_activex_plugin.SetPathPoints('0,0' + ',' + '0,100' + ',' + '100,100' + ',' + '100,0');
  } else if (null !== video_overlay_draw) {
    // video_overlay_draw.setvalue(g_rowcount, g_colcount, 'ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,ffffffff,fff00000', null);
    video_overlay_draw.setvalue(new fun_point('0', '0'), new fun_point('100', '0'), new fun_point('100', '100'), new fun_point('0', '100'));
  }
}

...
function fun_init_uicomponents() {
  sdk_getipcparam('/action/get?subject=devability', function (result) {
    ...
    $xml = $(result);
    var smartvaalg = 7; //parseInt($xml.find("smartva_alg").text());
    if (smartvaalg === 7) {
...
function fun_register_all_events() {
  ...
  $('#check_humandct_tagmask').click(function () {
    if ($('#check_humandct_tagmask').is(':checked')) {
      $('#check_humandct_showid').removeAttr('disabled');
    } else {
      $('#check_humandct_showid').attr('disabled', 'disabled');
...
function fun_init_draw_panel() {
  ...
  if (!CurBrowserIsIE()) {
    if (null == video_overlay_draw) {
      video_overlay_draw = new video_drawx('div_page_video_draw', VIDEO_DRAWX_COVER_TYPE_PICE);
      if (null != video_overlay_draw) {
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=persondetect', function (result) {
    ...
    var mask = '',
      ...
      showid = 0;
    if (g_algorithm_type === 2) {
      mask = $xml.find('mask').text();
      rows = $xml.find('row').text();
      cols = $xml.find('col').text();
    ...
    if (parseInt(cols) <= 0 || cols.length <= 0) {
      cols = 22;
    }
    if (parseInt(rows) <= 0 || rows.length <= 0) {
      rows = 18;
    }
    if (mask.length <= 0) {
      mask = '0';
    }
    if (CurBrowserIsIE() && g_algorithm_type === 2) {
      if (null !== g_activex_plugin) {
        //g_activex_plugin.SetPieceMaskFormat(rows,cols);
        //g_activex_plugin.SetPiecesMask(mask);
      }
    } else if (null !== video_overlay_draw && g_algorithm_type === 2) {
      video_overlay_draw.setvalue(rows, cols, mask, null);
    }
    g_rowcount = rows;
    g_colcount = cols;
...
function fun_on_base_save() {
  var drawmask, showmask, confidence, scenemode, showid;
  var strmask = '';
  if (g_algorithm_type === 2) {
    if (CurBrowserIsIE()) {
      //strmask = g_activex_plugin.GetPieceMask();
    } else {
      if (null != video_overlay_draw) {
        //drawmask = video_overlay_draw.getvalue().c;
      }
      var data = drawmask;
      for (var idx = 0; idx < data.length; idx++) {
        strmask += data[idx].toString(16);
 ...
 showmask = $('#check_humandct_tagmask').prop('checked') ? 1 : 0;
  ...
  if (g_algorithm_type === 2) {
    ...
    targetxml = '<?xml version="1.0" encoding="utf-8"?>' + '<request>' + '<persondetect ver="2.0">' + '<enable>1</enable>' + '<confidence>' + confidence + '</confidence>' + '<border>' + showmask + '</border>' + '<showid>' + showid + '</showid>' + '</persondetect>' + '</request>';
  }

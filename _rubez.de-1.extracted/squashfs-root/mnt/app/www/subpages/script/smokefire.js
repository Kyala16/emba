...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'sensitivity', 'div_confidence_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'confidenceval', 'div_humandct_confidence_name', ITEM_TYPE_TEXT);
  // translate_page_item(TARGET_PAGE_COMMON, 'enable', 'label_smokefire_tagmask_name', ITEM_TYPE_TEXT);
  translate_page_item(TARGET_PAGE_SUB_AVANALYSIS, 'targetmask', 'label_smokefire_tagmask_name', ITEM_TYPE_TEXT);
...
function fun_register_all_events() {
  ...
  $('#check_humandct_tagmask').click(function () {
    // if ($('#check_humandct_tagmask').prop('checked')) {
    //   $('#slider_humandct_confidence').slider('option', { disabled: false });
    // } else {
    //   $('#slider_humandct_confidence').slider('option', { disabled: true });
    // }
  });
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=32', function (result) {
    if (result != false) {
      $xml = $(result);
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=fireworks', function (res) {
    if (false === res) return;
    $xml = $(res);
    // var sensitive = $xml.find('sensitivity').text();
    //var enable = $xml.find('enable').text();

    // if (enable == 0) {
    //   $('#check_humandct_tagmask').prop('checked', false);
    //   $('#slider_humandct_confidence').slider('disable', { disabled: true });
    // } else {
    //   $('#check_humandct_tagmask').prop('checked', true);
    //   $('#slider_humandct_confidence').slider('disable', { disabled: false });
    // }
    // if (sensitive >= 0 && sensitive <= 3) {
    ...
    var border = parseInt($xml.find('border').text());
    $('#check_humandct_tagmask').prop('checked', (border ? true : false));
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=32', targetxml, function (result) {
...
function fun_on_base_save() {
  ...
  var targetxml =
    ...
    '</fireworks>\n' +
    '</request>';
  sdk_setipcparam('/action/set?subject=fireworks', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);
...
function fun_on_region_restore() {
  $('#select_intrusion_sensitivity').val(0);
  //$('#check_humandct_tagmask').prop('checked', false);
  //$('#slider_humandct_confidence').slider('value', 0);
  //$('#slider_humandct_confidence').attr('disabled', 'disabled');
  //$('#slider_humandct_confidence').slider('disable', { disabled: true });
  if (CurBrowserIsIE()) {
    if (null !== g_activex_plugin) {

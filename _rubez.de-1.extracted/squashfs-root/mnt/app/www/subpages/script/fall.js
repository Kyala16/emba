...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_COMMON, 'enable', 'label_humandct_tagmask_name', ITEM_TYPE_TEXT);
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=31', function (result) {
    if (result != false) {
      $xml = $(result);
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=fall', function (result) {
    if (false === result) {
      return;
    }
    $xml = $(result);
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=31', targetxml, function (result) {
...
function fun_on_base_save() {
  ...
  //var enable = $('#check_humandct_tagmask').prop('checked') ? 1 : 0;
  //var sensitivity = $('#slider_humandct_confidence').slider('value');
  // var points = null;
  // var tagrect = null;
  ...
  var targetxml =
    ...
    '</falldetect>\n' +
    '</request>';
  sdk_setipcparam('/action/set?subject=fall', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);

...
function fun_multilang_adapter() {
  ...
  translate_page_item(TARGET_PAGE_SUB_MOTION, 'rsio', 'label_action_motion_rsio', ITEM_TYPE_VALUE);//485
...
function fun_get_schedule_parameters() {
  ...
  sdk_getipcparam('/action/get?subject=alarm&type=14', function (result) {
    if (result != false) {
      $xml = $(result);
...
function fun_get_base_parameters() {
  sdk_getipcparam('/action/get?subject=leftremoved', function (res) {
    if (false === res) return;
    $xml = $(res);
...
function fun_save_schedule() {
  ...
  var targetxml =
    ...
    '</alarmevt>' +
    '</request>';
  sdk_setipcparam('/action/set?subject=alarm&type=14', targetxml, function (result) {
...
function fun_on_base_save() {
  ...
  var targetxml =
    ...
    '</leftremoved>\n' +
    '</request>';
  sdk_setipcparam('/action/set?subject=leftremoved', targetxml, function (result) {
    var strsuc = translate_page_item(TARGET_PAGE_TIPSTEXT, 'infosave', '', ITEM_TYPE_NONE);
    var failed = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errset', '', ITEM_TYPE_NONE);
    var auth = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errauthority', '', ITEM_TYPE_NONE);
    var request = translate_page_item(TARGET_PAGE_TIPSTEXT, 'errrequest', '', ITEM_TYPE_NONE);
...
function fun_on_action_save() {
  ...
  var actionemulation = document.getElementById('check_action_motionemulation').checked;
  ...
  //   schedule_action_mask &= ~(1 << 19);
  // }
  var actionrsio = document.getElementById('check_action_rsio').checked;//485
  if (actionrsio) {//485
    schedule_action_mask |= 1 << 19;
  } else {
    schedule_action_mask &= ~(1 << 19);
